...
namespace NeuronDotNet.Core.PSO
{
        ...
        : Connector<ActivationLayer, ActivationLayer,PSOSynapse>
    {
        ...
        /// </exception>
        public PSOConnector(ActivationLayer sourceLayer, ActivationLayer targetLayer)
        ...
        /// If <c>sourceLayer</c> or <c>targetLayer</c> is <c>null</c>
        /// </exception>
        public PSOConnector(ActivationLayer sourceLayer, ActivationLayer targetLayer, ConnectionMode connectionMode)
        ...
        private void ConstructENFORMSynapses()
        {
            ...
            if (connectionMode == ConnectionMode.Complete)
            {
                        ...
                        if (targetLayer.InputGroups != -1)
                        {
                            ...
                            foreach (ENFORM.InputGroup inputGroup in inputGroups)
                            {
                                ...
                                foreach (ActivationNeuron sourceNeuron in sourceLayer.Neurons)
                                {
                                    ...
                                    synapses[synapseCount++] = new PSOSynapse(sourceNeuron, targetNeurons[totalTargetNeuron + segment], this);
                                }
                        ...
                        else
                        {
                            ...
                            foreach (ActivationNeuron targetNeuron in targetLayer.Neurons)
                            {
                                foreach (ActivationNeuron sourceNeuron in sourceLayer.Neurons)
                                {
                                    synapses[synapseCount++] = new PSOSynapse(sourceNeuron, targetNeuron, this);
                                }
            ...
            else
            {
                ...
                {
                    synapses[synapseCount++] = new PSOSynapse(
        ...
        private void ConstructSynapses()
        {
            ...
            if (connectionMode == ConnectionMode.Complete)
            {
                foreach (ActivationNeuron targetNeuron in targetLayer.Neurons)
                {
                    foreach (ActivationNeuron sourceNeuron in sourceLayer.Neurons)
                    {
                        synapses[i++] = new PSOSynapse(sourceNeuron, targetNeuron, this);
                    }
                }
